...
- [x] 8.5 Add test: folder-scoped search returns only scoped chunks (set up 2 folders, search with scope, verify filtering)
...
- [x] 8.6 Add test: chunk `folder_id` updated when knowledge entry is moved to different folder
...
- [x] 8.8 Add test: folder upload atomicity — simulate unique-name conflict mid-batch → full rollback
...
- [x] 8.12 Manual smoke test: create nested folders, upload a local folder, upload a ZIP, move entries, search within a folder
