...
// for a knowledge document. This is used by post-process stages that need the full text content.
func (s *chunkService) ListAllContentChunksByKnowledgeID(ctx context.Context, knowledgeID string) ([]*types.Chunk, error) {
	tenantID := types.MustTenantIDFromContext(ctx)
	chunks, err := s.chunkRepository.ListAllContentChunksByKnowledgeID(ctx, tenantID, knowledgeID)
	if err != nil {
		logger.ErrorWithFields(ctx, err, map[string]interface{}{
			"knowledge_id": knowledgeID,
			"tenant_id":    tenantID,
	...
	logger.Infof(ctx, "Retrieved %d content chunks successfully", len(chunks))
	return chunks, nil
}
