...
| 语雀 (Yuque) | API Key (Token) | ✅ | — |
| RSS / Atom Feed | 无需认证（Feed URL） | ✅ | — |
| SVN 仓库 | 用户名/密码（可选匿名） | ✅ | ✅ |

> 上表为容器实际注册的连接器（`initConnectorRegistry`）；元数据表中另有 Confluence、GitHub、IMAP 等预留条目，尚未注册可用。
...
1. 确认运行环境已安装 `svn` CLI（官方 Docker 镜像已内置；裸机部署需 `apt-get install subversion`）
2. 在知识库设置页添加数据源，选择 **SVN Repository**
3. 填写仓库地址（支持 `svn://`、`http(s)://`、`svn+ssh://`，不支持 `file://`）与可选用户名/密码
4. 测试连接，配置同步策略与可选设置（扩展名白名单 `file_extensions`、排除路径 `exclude_paths`、单文件上限 `max_file_size`，默认 50 MB）
5. 触发首次同步验证

> SVN 连接器不做本地检出，通过 `svn` CLI 远程读取；增量同步基于 `svn diff --summarize` 的 revision 游标，支持删除同步。所有配置均存数据库（AES-256-GCM 加密），无需配置环境变量。
