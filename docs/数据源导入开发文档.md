...
从 SVN 仓库同步文档到 WeKnora。连接器通过 `svn` CLI 远程读取，**无需本地检出**。

#### 第一步：确认 svn CLI 可用

- **Docker 部署**：官方镜像已内置 `subversion` 包，无需额外操作
- **裸机部署**：需自行安装，如 `apt-get install subversion`（Debian/Ubuntu）或 `yum install subversion`（RHEL/CentOS）

#### 第二步：在 WeKnora 中添加数据源

1. 进入知识库设置页 → **数据源** 标签页 → **添加数据源**
2. 选择连接器类型：**SVN Repository**
3. 填写凭证（Credentials）：
   - **仓库地址 (repo_url)**：SVN 仓库根 URL，支持 `svn://`、`http://`、`https://`、`svn+ssh://` 协议（`file://` 不支持）
   - **用户名 / 密码**：可选；匿名访问的仓库可留空
4. 点击 **测试连接**（底层执行 `svn info --xml` 验证连通性与凭证）
5. 选择仓库资源，配置同步策略后保存
...
| 字段 | 说明 | 默认 |
|------|------|------|
| `file_extensions` | 文件扩展名白名单（如 `[".md", ".txt"]`），空 = 同步全部类型 | 空 |
| `exclude_paths` | 排除路径 glob 列表（如 `["draft/*", "*/temp/*"]`） | 空 |
| `max_file_size` | 单文件大小上限（字节），超限文件静默跳过 | 50 MB |

> **删除同步**：SVN 增量同步基于 `svn diff --summarize -r OLD:HEAD` 精确识别新增/修改/删除，开启「同步删除」后源端删除的文件会同步下线。
...
SVN 连接器通过调用 `svn` 命令行工具从 SVN 仓库同步文档，**不做本地检出**（checkout-less）。选择 CLI 封装而非 Go 库的原因：Go 生态无成熟的 libsvn 绑定，候选库或缺核心命令或为单人停更项目（详见 `openspec/changes/archive/2026-07-14-add-svn-connector/design.md` D1）。

#### 认证机制

- 用户名/密码通过 argv 传入（`--username` / `--password`），不经 shell，无注入面
- 每次调用附加 `--non-interactive --no-auth-cache`：
  - 非交互：认证失败立即报错，而非挂起等待 stdin 输入
  - 不缓存：凭证不落盘到 `~/.subversion/auth`（多空间服务器上的凭证卫生）
- 错误消息只含子命令名，不含原始 argv——凭证值不会泄露进日志
- 匿名仓库可完全省略用户名/密码

#### 资源发现

`ListResources` 将仓库根映射为单个 `Resource`（SVN 没有「空间」概念），返回信息来自 `svn info --xml`（HEAD revision、repo UUID、最后提交时间）。

#### 全量同步 (FetchAll)

```
1. svn info --xml → 获取 HEAD revision 与 repo UUID
2. svn list -R --xml → 递归列出仓库全部条目
3. shouldInclude 过滤：
   ├─ file_extensions 白名单（空 = 全部类型）
   ├─ exclude_paths glob 排除
   └─ size > max_file_size → 跳过
4. svn cat -r HEAD 逐个读取内容
   └─ 有界读取：输出超过 max_file_size 立即终止进程并软跳过
```

#### 增量同步 (FetchIncremental)

```
1. 加载游标 {last_revision, repo_uuid}
2. svn info → HEAD；HEAD == last_revision → 无变更，直接返回
3. svn diff --summarize -r last:HEAD --xml → 精确变更列表
   ├─ A (Added) / M (Modified) → svn cat 拉取内容
   └─ D (Deleted) → 标记 IsDeleted（删除同步）
4. 返回新游标 {HEAD, repo_uuid}
```

- **游标基于 revision 号而非时间戳**：SVN revision 全序单调，无漏同步/重复同步风险
- **repo UUID 校验**：仓库重建（UUID 变更）后旧游标失效，服务层回退全量同步

#### 安全与资源保护

| 机制 | 说明 |
|------|------|
| 参数注入防护 | 参数全部经 argv 传递（无 shell 拼接），`--` 分隔选项与路径 |
| 输出有界 | `io.LimitReader` 截断至 max_file_size，超限杀进程，防巨型文件撑爆内存 |
| 超时分级 | info/list/diff 5 min；list -R 15 min；cat 按 max_file_size 以 100 KB/s 保守吞吐推导（5–30 min） |
| 进程组清理 | 超时杀整个进程组（覆盖 svn+ssh 拉起的 ssh 孙进程），`Setpgid` + `cmd.Cancel` + `WaitDelay` |
| 软跳过一致性 | 超限文件在全量/增量两条路径均静默跳过（记日志），不报部分失败 |

#### 源码文件

| 文件 | 职责 |
|------|------|
| `internal/datasource/connector/svn/types.go` | Config、svnCursor、XML DTO、`svnCLI` 接口（可 mock） |
| `internal/datasource/connector/svn/client.go` | svn CLI 封装：info/list/cat/diff、有界输出、超时分级、错误卫生 |
| `internal/datasource/connector/svn/procattr_unix.go` / `procattr_other.go` | 进程组杀（unix 实现 / 其他平台回退） |
| `internal/datasource/connector/svn/connector.go` | Connector 接口实现：Validate、ListResources、FetchAll、FetchIncremental |
| `internal/datasource/connector/svn/*_test.go` | 单元测试（mock CLI）+ 集成测试（真实 svnserve 仓库，`-tags=integration`） |
